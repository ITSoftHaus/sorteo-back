    ...
    createRouter(router) {
        ...
        return router()
            ...
            .get(
                ...
                '/lotofacil/loadzipfile', async (_req: Request, res: Response) => {
                    try {
                        res.send({ dataLoadedSorteiosLotofacil: SorteoCtrl.getZipFileFromUrl() });
                    } catch (error) {
                        console.error(error);
                    }                    
                }
            ).get(
                '/lotofacil/loaded', (_req: Request, res: Response) => {
                    res.send({ dataLoadedSorteiosLotofacil: SorteoCtrl.getLoadedDataFromLotofacil() })
                }
            ).get(
                '/lotofacil/loaded/buffer', (_req: Request, res: Response) => {
                    res.send({ bufferSorteiosLotofacil: SorteoCtrl.getLoadedFileBufferFromLotofacil().toString() });
                }
            ).get(
                '/lotofacil/loaded/model', (_req: Request, res: Response) => {
                    res.send({ modelLoadedSorteiosLotofacil: SorteoCtrl.getLotofacilModels() })
                }
            ).get(
                '/lotofacil/loaded/mining/totalizador/numerossorteadosbola/:bola', (_req: Request, res: Response) => {
                    res.send({ bolasSorteadas: SorteoCtrl.getLotofacilDataMining.getNumerosSorteadoPorBola(_req.params.bola) })
                }
            ).get(
                '/lotofacil/loaded/mining/totalizador/calcularnumerossorteadosbola/:bola', (_req: Request, res: Response) => {
