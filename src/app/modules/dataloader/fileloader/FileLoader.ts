...
export default class FileLoader implements IFileLoader {
  ...
  public async getZipFileFromUrl(url: string): Promise<String> {
    const client = this.http;
    const response = await client.get(url);
    const filePath = this.filePath;
    const file: NodeJS.WritableStream = fs.createWriteStream(
      path.resolve(__dirname + '/../../../../../files/sorteo.zip')
    );
  ...
  private validaResponse(response: IHttpClientResponse) {
    if (response.message.statusCode !== 200) {
      ...
      const err: Error = new Error(
        `Resposta HTTP inesperada: ${response.message.statusCode}`
      );
