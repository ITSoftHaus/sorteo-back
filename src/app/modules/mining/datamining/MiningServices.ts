...
export default class MiningServices {
  ...
  protected contabilizaPorcentagem({
    numerosSorteadosPorBola,
    total
  }: {
    numerosSorteadosPorBola: Totalizador[];
    total: number;
  }) {
  ...
  protected contabilizaNumerosSorteadosPorBola({
    numerosSorteadosPorBolaData,
    numerosSorteadosPorBola
  }: {
    numerosSorteadosPorBolaData: string[];
    numerosSorteadosPorBola: Totalizador[];
  }) {
    this.contabilizarNumerosSorteadosPorBola({
      numerosSorteadosPorBolaData,
      numerosSorteadosPorBola
    });
  }

  protected formatarModeloInicial({
    numerosSorteadosPorBolaData,
    numerosSorteadosPorBola
  }: {
    numerosSorteadosPorBolaData: string[];
    ...
    numerosSorteadosPorBola.sort((a, b) =>
      a.getNumero() > b.getNumero() ? 1 : -1
    );
  }

  protected contabilizarNumerosSorteadosPorBola({
    numerosSorteadosPorBolaData,
    numerosSorteadosPorBola
  }: {
    numerosSorteadosPorBolaData: string[];
    ...
    Object.keys(numerosSorteadosPorBolaData).forEach(key => {
      ...
      const index = numerosSorteadosPorBola.findIndex(
        (e: { getNumero(): number }) => {
